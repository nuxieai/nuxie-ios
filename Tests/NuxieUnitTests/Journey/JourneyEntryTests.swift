...
final class JourneyEntryTests: XCTestCase {
    private struct Vector: Decodable {
        struct Event: Decodable { let name: String; let properties: [String: AnyCodable] }
        let name: String
        let condition: JourneyEntryCondition
        let facts: JourneyFactTable
        let references: JourneyFactReferences
        let foreground: Bool
        let event: Event?
        let expected: Bool
    ...
    func testHistoryPredicateUsesItsQueryEventWithoutAnEntryEvent() async {
        ...
        let condition = JourneyEntryCondition(type: .appForegrounded, eventName: nil, segmentId: nil, member: nil,
        ...
        let matches = await JourneyEntryEvaluator.matches(condition,
