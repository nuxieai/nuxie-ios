...
final class FlowArtifactStoreTests: AsyncSpec {
    override class func spec() {
        func writeFixtureArtifact(
            ...
            let rivSha = ExperienceArtifactStore.sha256Hex(rivData)
            ...
            let imageData = includeImageAsset
                ...
                : nil
            let imageSha = imageData.map(ExperienceArtifactStore.sha256Hex)
            ...
            let fontSha = fontData.map(ExperienceArtifactStore.sha256Hex)
            ...
            let buildManifest = BuildManifest(
                ...
                contentHash: ExperienceArtifactStore.sha256Hex(contentHashData),
                files: buildFiles
            )
        ...
        func replacingBuildFileSize(
            ...
            let artifact = flow.screens.flowArtifact
            ...
            return Experience(
                screens: RemoteFlow(
                    ...
                    handlers: screens.handlers,
                    scripts: screens.scripts,
                    viewModelValues: screens.viewModelValues,
                    responseSchemas: screens.responseSchemas
                ),
        ...
        func replacingBuildManifest(
            ...
            let screens = flow.screens
            let artifact = screens.flowArtifact
            return Experience(
                screens: RemoteFlow(
                    id: screens.id,
                    flowArtifact: FlowArtifact(
        ...
        describe("ExperienceArtifactStore") {
            ...
            it("rejects inconsistent build-manifest file metadata") {
                let fixture = try writeFixtureArtifact()
                let manifest = fixture.flow.screens.flowArtifact.manifest
                let flow = replacingBuildManifest(
                    in: fixture.flow,
                    with: BuildManifest(
            ...
            it("rejects aggregate declared bytes over the fixed build envelope") {
                ...
                let oversizedFile = BuildFile(
                    path: "unused.bin",
                    size: ExperienceArtifactStore.maximumBuildDeclaredBytes + 1,
                    contentType: "application/octet-stream"
                )
                let flow = replacingBuildManifest(
                    in: fixture.flow,
                ...
                await expect {
                    ...
                    throwError(
                        FlowRuntimeImportValidationError.valueExceedsLimit(
                            ...
                            limit: ExperienceArtifactStore.maximumBuildDeclaredBytes
                        )
                    )
                )
            }
            ...
            it("rejects an oversized declared manifest before reading its remote file") {
                ...
                try FileManager.default.removeItem(
                    at: fixture.baseURL.appendingPathComponent(ExperienceArtifactStore.manifestPath)
                )
                let store = ExperienceArtifactStore(
                    cacheDirectory: fixture.cacheURL,
                    runtimeAssetStore: RuntimeAssetStore(cacheDirectory: fixture.runtimeCacheURL)
            ...
            it("cancels a remote build-file response when its body exceeds its declaration") {
                ...
                let manifestData = try Data(
                    contentsOf: fixture.baseURL.appendingPathComponent(ExperienceArtifactStore.manifestPath)
                )
                let oversizedRivData = fixture.rivData + Data([0])
                StubURLProtocol.reset()
                defer { StubURLProtocol.reset() }
                StubURLProtocol.registerSuccess(
                    path: remoteURL.appendingPathComponent(ExperienceArtifactStore.manifestPath).path,
                    data: manifestData,
            ...
            it("keeps visual import available when a listed signature file is missing") {
                ...
                let flow = appendingBuildFile(
                    to: fixture.flow,
                    file: BuildFile(
                        path: ExperienceArtifactStore.manifestSignaturePath,
                        size: 128,
                        contentType: "application/json"
                    )
                )
            ...
            it("coordinates invalid image-cache replacement across store actors") {
                ...
                let asset = FlowArtifactImageAsset(
                    ...
                    path: imagePath,
                    sha256: ExperienceArtifactStore.sha256Hex(expectedData),
                    contentType: "image/png",
                    width: 1,
                    height: 1,
                    required: true
            ...
            it("does not delete a valid font cache when the work budget is exhausted") {
                ...
                let asset = FlowArtifactFontAsset(
                    ...
                    style: "normal",
                    assetUrl: sourceURL.absoluteString,
                    sha256: ExperienceArtifactStore.sha256Hex(data),
            ...
            it("bounds a downloaded font response by its declared size") {
                ...
                let asset = FlowArtifactFontAsset(
                    ...
                    style: "normal",
                    assetUrl: fontURL.absoluteString,
                    sha256: ExperienceArtifactStore.sha256Hex(fontData),
                    sizeBytes: 8,
                    contentType: "font/ttf",
                    format: "ttf",
                    required: true
            ...
            it("does not let a missing optional image poison valid content with the same hash") {
                ...
                var flow = try rewritingRemoteManifest(
                    ...
                    let firstImage: [String: Any] = [
                        ...
                        "sha256": ExperienceArtifactStore.sha256Hex(imageData),
                        "contentType": "image/png",
                        "width": 1,
                        "height": 1,
                        "required": false,
            ...
            it("validates each image descriptor before reusing verified content") {
                ...
                let loaded = try await ExperienceArtifactStore(
                    cacheDirectory: fixture.cacheURL,
                    runtimeAssetStore: RuntimeAssetStore(
                        cacheDirectory: fixture.runtimeCacheURL
                    )
            ...
            it("validates font size format and content type before content reuse") {
                ...
                let validFontURL = fixture.baseURL
                    .appendingPathComponent("external-fonts/test-font.ttf")
                let hash = ExperienceArtifactStore.sha256Hex(fontData)
                let flow = try rewritingRemoteManifest(
                    in: fixture.flow,
                    at: fixture.baseURL
                ) { object in
            ...
            it("coalesces a caller that arrives after the manifest is downloaded") {
                ...
                let flow = replacingBuildManifest(
                    in: fixture.flow,
                    with: fixture.flow.screens.flowArtifact.manifest,
                    baseURL: baseURL
                )
                let manifestData = try Data(
            ...
            it("coordinates publication across independent stores sharing a cache") {
                ...
                StubURLProtocol.registerSuccess(
                    path: "/shared-store-cache/\(ExperienceArtifactStore.manifestPath)",
                    data: manifestData,
                    headers: ["Content-Length": "\(manifestData.count)"]
                )
                StubURLProtocol.register(
                ...
                let firstStore = ExperienceArtifactStore(
                    urlSession: session,
                    cacheDirectory: fixture.cacheURL,
                    runtimeAssetStore: RuntimeAssetStore(
                        cacheDirectory: fixture.runtimeCacheURL
                ...
                let secondStore = ExperienceArtifactStore(
                    urlSession: session,
                    cacheDirectory: fixture.cacheURL,
                    runtimeAssetStore: RuntimeAssetStore(
                        cacheDirectory: fixture.runtimeCacheURL
            ...
            it("cancels and cleans a matching active download when removed") {
                ...
                StubURLProtocol.registerSuccess(
                    path: "/remove-active/\(ExperienceArtifactStore.manifestPath)",
                    data: manifestData,
                    headers: ["Content-Length": "\(manifestData.count)"]
                )
                StubURLProtocol.register(
            ...
            it("cancels and cleans every active download when cleared") {
                ...
                StubURLProtocol.registerSuccess(
                    path: "/clear-active/\(ExperienceArtifactStore.manifestPath)",
                    data: manifestData,
                    headers: ["Content-Length": "\(manifestData.count)"]
                )
                StubURLProtocol.register(
            ...
            it("clears the canonical cache without replacing a configured root symlink") {
                ...
                let freshStore = ExperienceArtifactStore(
                    cacheDirectory: aliasURL,
                    runtimeAssetStore: RuntimeAssetStore(
                        cacheDirectory: fixture.runtimeCacheURL
                    )
            ...
            it("rejects unsafe manifest paths") {
                expect {
                    try ExperienceArtifactStore.validateRelativePath("../flow.riv")
                }.to(throwError())
            }
        }
    }
