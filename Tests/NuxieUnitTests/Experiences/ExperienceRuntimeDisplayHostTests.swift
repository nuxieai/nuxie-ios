...
final class ExperienceRuntimeDisplayHostTests: AsyncSpec {
    override class func spec() {
        describe("ExperienceRuntimeDisplayHost lifecycle and input") {
            it("coalesces byte-exact text runs in FIFO order before one zero-delta render") { @MainActor in
                let firstTextResult = ExperienceRuntimeOperationResult(
                ...
                let secondTextResult = ExperienceRuntimeOperationResult(
                    renderOutcome: .notRequested,
                    isDirty: true,
                    isSettled: false,
                    diagnostics: [
                ...
                let renderResult = ExperienceRuntimeOperationResult(
                ...
                var completions: [ExperienceRuntimeOperationResult] = []
                var delivered: [ExperienceRuntimeOperationResult] = []
            ...
            it("ignores display ticks after the runtime settles") { @MainActor in
                let settled = ExperienceRuntimeOperationResult(
                    renderOutcome: .presented,
                    surfaceDisposition: .presented,
                    isDirty: false,
                    isSettled: true
            ...
            it("wakes a settled visible session at the runtime deadline") { @MainActor in
                let sleeping = ExperienceRuntimeOperationResult(
                    renderOutcome: .presented,
                    surfaceDisposition: .presented,
                    isDirty: false,
                    isSettled: true,
                ...
                let woke = ExperienceRuntimeOperationResult(
                    renderOutcome: .presented,
                    surfaceDisposition: .presented,
                    isDirty: false,
                    isSettled: true
            ...
            it("advances an active detached session without a drawable until settled") { @MainActor in
                ...
                let recorder = FakeExperienceRuntimeLifecycleRecorder()
            ...
            it("wakes a settled detached session for a zero-delta mutation flush") { @MainActor in
                ...
                let mutation = ExperienceRuntimeOperationResult(
                    renderOutcome: .notRequested,
                    isDirty: true,
                    isSettled: true
                )
                let flushed = ExperienceRuntimeOperationResult(
                    renderOutcome: .notRequested,
                    isDirty: false,
                    isSettled: true
                )
                ...
                host.performStateBatch(
                    prepare: { ExperienceRuntimeStateBatch(mutations: []) }
                )
            ...
            it("delivers projected results through the one-argument result sink") { @MainActor in
                let original = ExperienceRuntimeOperationResult(
                    renderOutcome: .presented,
                    surfaceDisposition: .presented,
                    isDirty: true,
                    isSettled: false,
                ...
                let projected = ExperienceRuntimeOperationResult(
                    renderOutcome: .presented,
                    surfaceDisposition: .presented,
                    isDirty: false,
                    isSettled: true,
            ...
            it("keeps a failed text run control-local and continues before pointer and frame work") { @MainActor in
                let textResult = ExperienceRuntimeOperationResult(
                    renderOutcome: .notRequested,
                    isDirty: true,
                    isSettled: false
                )
                ...
                let pointerResult = ExperienceRuntimeOperationResult(
                    renderOutcome: .notRequested,
                    isDirty: true,
                    isSettled: false
                )
                let frameResult = ExperienceRuntimeOperationResult(
                    renderOutcome: .presented,
                    surfaceDisposition: .presented,
                    isDirty: false,
                    isSettled: true
                ...
                let adapter = FakeExperienceRuntimeAdapter(operationResults: [
                    .failure(ExperienceRuntimeDisplayHostLocalTestError.failed),
                    .success(textResult),
                    .success(renderResult),
            ...
            it("poisons the lane only when a text failure proves the session terminal") { @MainActor in
                let adapter = FakeExperienceRuntimeAdapter(operationResults: [
                    .failure(ExperienceRuntimeDisplayHostUnexpectedTestError.failed),
                ])
                let context = try await ExperienceRuntimeContextFactory(adapter: adapter).makeContext(
                    for: ExperienceRuntimeImportRequest.testStub(packageBytes: Data([0x52, 0x49, 0x56]))
                )
            ...
            it("notifies the terminal owner before flushing unprepared queued completions") { @MainActor in
                ...
                host.performStateBatch(
                    prepare: {
                        ...
                        return ExperienceRuntimeStateBatch(mutations: [])
                    },
            ...
            it("reports a terminal deferred state-preparation failure before draining the queue") { @MainActor in
                ...
                host.performStateBatch(
                    prepare: {
                        events.append("first prepared")
                        throw ExperienceRuntimeDisplayHostUnexpectedTestError.failed
                    },
                    completion: { result in
                        if case .failure = result { events.append("first completion") }
                    }
            ...
            it("continues after an explicitly local deferred state-preparation failure") { @MainActor in
                let adapter = FakeExperienceRuntimeAdapter(operationResults: [
                    .success(ExperienceRuntimeOperationResult(
                        renderOutcome: .notRequested,
                        isDirty: false,
                        isSettled: true
                    )),
                ...
                host.performStateBatch(
                    prepare: {
                        events.append("first prepared")
                        throw ExperienceRuntimeDisplayHostLocalTestError.failed
                    },
                    completion: { result in
                        if case .failure = result { events.append("first completion") }
                    }
            ...
            it("defers FIFO state preparation until earlier runtime results are delivered") { @MainActor in
                ...
                let stateResult = ExperienceRuntimeOperationResult(
                    renderOutcome: .notRequested,
                    isDirty: true,
                    isSettled: false,
                    diagnostics: [
            ...
            it("runs state, text, and logical work detached before rendering on reattach") { @MainActor in
                let mutationResult = ExperienceRuntimeOperationResult(
                    renderOutcome: .notRequested,
                    isDirty: true,
                    isSettled: false
                )
                let offscreenFlushResult = ExperienceRuntimeOperationResult(
                    renderOutcome: .notRequested,
                    isDirty: false,
                    isSettled: true
                )
                ...
                host.performStateBatch(prepare: {
                    ExperienceRuntimeStateBatch(mutations: [
                        .setInputBool(name: "enabled", value: true),
                    ])
                })
            ...
            it("bounds total queued host work including coalesced text completions") { @MainActor in
                ...
                for index in 0..<ScreenSessionLimits.batchItems {
                    host.setText("\(index)", forRunNamed: "Headline")
                }
                var overflow: ExperienceRuntimeDisplayHostError?

                host.performStateBatch(
                    prepare: { ExperienceRuntimeStateBatch(mutations: []) },
                    completion: { result in
                        guard case .failure(let error) = result else { return }
                        overflow = error as? ExperienceRuntimeDisplayHostError
                    }
                )

                expect(overflow).to(equal(.pendingHostWorkOverflow(
                    limit: ScreenSessionLimits.batchItems
                )))
                if let overflow {
                    expect(screenSessionOperationFailureInvalidatesSession(overflow)).to(beFalse())
                }
                expect(
                    adapter.contextDrivers.first?.sessionDrivers.first?.performedOperations ?? []
                ).to(beEmpty())
            ...
            it("maps touch input through authored bounds and ends with one up event") { @MainActor in
                ...
                let touch = NSObject()
                let source = ExperienceRuntimePointerSourceID(touch)
            ...
            it("keeps pointer batches FIFO while giving a pending frame a turn") { @MainActor in
                ...
                let adapter = FakeExperienceRuntimeAdapter(operationResults: Array(
                ...
                host.runtimeSurfaceViewDidReceivePointerEvents([
                    ExperienceRuntimeViewPointerEvent(
                        source: ExperienceRuntimePointerSourceID(firstSource),
                        kind: .down,
                ...
                host.runtimeSurfaceViewDidReceivePointerEvents([
                    ExperienceRuntimeViewPointerEvent(
                        source: ExperienceRuntimePointerSourceID(secondSource),
                        kind: .move,
                ...
                expect(firstFrame).to(equal(ExperienceRuntimeFrameTime(timestamp: 1, delta: 0)))
                ...
                expect(secondFrame).to(equal(ExperienceRuntimeFrameTime(timestamp: 2, delta: 1)))
            ...
            it("drops saturated lifecycle input without poisoning later frames") { @MainActor in
                ...
                host.displayLinkDidFire(at: 1)
                let admittedSources = sources.prefix(ScreenSessionLimits.pointerEvents)
                host.runtimeSurfaceViewDidReceivePointerEvents(
                ...
                let overflowSource = ExperienceRuntimePointerSourceID(
                ...
                let overflow = ExperienceRuntimeDisplayHostError.pendingPointerInputOverflow(
                    limit: ScreenSessionLimits.pointerEvents * 2
                )
                expect(screenSessionOperationFailureInvalidatesSession(overflow)).to(beFalse())
            ...
            it("drains queued input detached before a zero-time render on reattach") { @MainActor in
                ...
                let recorder = FakeExperienceRuntimeLifecycleRecorder()
                let completionGate = FakeExperienceRuntimeDrawableCompletionGate()
                let adapter = FakeExperienceRuntimeAdapter(
                    operationResults: [
                        .success(frameResult),
                        .success(pointerResult),
            ...
            it("uses a nonblocking bounded drawable budget") { @MainActor in
                let gate = ExperienceRuntimeDrawableGate(capacity: 2)
            ...
            it("acquires and forwards drawables while reusing permits after presented and skipped frames") { @MainActor in
                let presented = ExperienceRuntimeOperationResult(
                    renderOutcome: .presented,
                    surfaceDisposition: .presented,
                    isDirty: true,
                    isSettled: false
                )
                let skipped = ExperienceRuntimeOperationResult(
                ...
                guard let (window, view) = makeConfiguredMetalSurface() else { return }
                let gate = ExperienceRuntimeDrawableGate(capacity: 1)
                let host = ExperienceRuntimeDisplayHost(
                    session: session,
                    surfaceView: view,
                    drawableGate: gate,
            ...
            it("recovers device loss through one detach, refreshed reattach, and zero-delta redraw") { @MainActor in
                let deviceLost = ExperienceRuntimeOperationResult(
                    renderOutcome: .skipped,
                    surfaceDisposition: .deviceLost,
                    isDirty: true,
                    isSettled: false
                )
                let recovered = ExperienceRuntimeOperationResult(
                    renderOutcome: .presented,
                    surfaceDisposition: .presented,
                    isDirty: false,
                    isSettled: true
            ...
            it("fails closed when device loss repeats before the recovery frame succeeds") { @MainActor in
                ...
                expect(receivedErrors.first as? ExperienceRuntimeHostError).to(
            ...
            it("fails queued work if its surface view disappears during device-loss recovery") { @MainActor in
                ...
                expect(receivedError as? ExperienceRuntimeHostError).to(
                    equal(.disposedSurface)
                )
                expect(completionError as? ExperienceRuntimeHostError).to(
                    equal(.disposedSurface)
                )
            ...
            it("finishes recovery before queued host work and its completion") { @MainActor in
                ...
                let recoveryFrame = ExperienceRuntimeOperationResult(
                    renderOutcome: .presented,
                    surfaceDisposition: .presented,
                    isDirty: false,
                    isSettled: true
                ...
                let textFrame = ExperienceRuntimeOperationResult(
                    renderOutcome: .presented,
                    surfaceDisposition: .presented,
                    isDirty: false,
                    isSettled: true
                ...
                guard let (window, view) = makeConfiguredMetalSurface() else { return }
                var delivered: [ExperienceRuntimeOperationResult] = []
                var textCompletions: [ExperienceRuntimeOperationResult] = []
                let host = ExperienceRuntimeDisplayHost(
                    session: session,
                    surfaceView: view,
                    usesSystemDisplayLink: false,
            ...
            it("recovers while hidden before continuing offscreen host work") { @MainActor in
                ...
                let settledAdvance = ExperienceRuntimeOperationResult(
                    renderOutcome: .notRequested,
                    isDirty: false,
                    isSettled: true
                )
                ...
                guard let (window, view) = makeConfiguredMetalSurface() else { return }
                var delivered: [ExperienceRuntimeOperationResult] = []
                var stateCompletion: Result<ExperienceRuntimeOperationResult, Error>?
                var receivedErrors: [Error] = []
                let host = ExperienceRuntimeDisplayHost(
                    session: session,
                    surfaceView: view,
            ...
            it("isolates device recovery to the affected sibling session") { @MainActor in
                ...
                let firstRecovered = ExperienceRuntimeOperationResult(
                    renderOutcome: .presented,
                    surfaceDisposition: .presented,
                    isDirty: false,
                    isSettled: true
                )
                let siblingFrame = ExperienceRuntimeOperationResult(
                    renderOutcome: .presented,
                    surfaceDisposition: .presented,
                    isDirty: true,
                    isSettled: false
                ...
                let context = try await ExperienceRuntimeContextFactory(
                    adapter: adapter
                ).makeContext(
                    for: ExperienceRuntimeImportRequest.testStub(packageBytes: Data([0x52, 0x49, 0x56]))
                )
                ...
                var firstDelivered: [ExperienceRuntimeOperationResult] = []
                var siblingDelivered: [ExperienceRuntimeOperationResult] = []
                let firstHost = ExperienceRuntimeDisplayHost(
                    session: firstSession,
                    surfaceView: firstView,
                    usesSystemDisplayLink: false,
                    onResult: { firstDelivered.append($0) }
                )
                let siblingHost = ExperienceRuntimeDisplayHost(
            ...
            it("does not acquire a stale drawable for a zero-sized logical surface") { @MainActor in
                ...
                let resized = await waitUntil {
                    recorder.events.contains(
                        .surfaceResized(
                            ExperienceRuntimeSurfaceSize(pixelWidth: 0, pixelHeight: 0)
                        )
                    )
                }
            ...
            it("fails startup and resolves queued work when its weak surface view disappears") { @MainActor in
                ...
                var surfaceView: ExperienceRuntimeSurfaceView? = ExperienceRuntimeSurfaceView(frame: .zero)
                ...
                expect(startError as? ExperienceRuntimeHostError).to(equal(.disposedSurface))
                expect(reportedError as? ExperienceRuntimeHostError).to(equal(.disposedSurface))
                expect(completionError as? ExperienceRuntimeHostError).to(equal(.disposedSurface))
            }
            ...
            it("reports an attachment failure and resolves every queued host request") { @MainActor in
                ...
                expect(startError as? ExperienceRuntimeHostError).to(equal(.disposedSession))
                expect(reportedError as? ExperienceRuntimeHostError).to(equal(.disposedSession))
                expect(completionErrors.compactMap { $0 as? ExperienceRuntimeHostError }).to(
...
private func waitForOperationCount(
    _ count: Int,
    driver: FakeScreenSessionDriver
) async -> Bool {
