...
final class ExperienceRuntimeStateBridgeTests: XCTestCase {
    func testSnapshotBecomesOneTypedBatchAgainstTheScreenRootInstance() throws {
        ...
        let coordinator = ExperienceViewModelStateCoordinator(screens: fixture.screens)
        let bridge = try ExperienceRuntimeStateBridge(
        ...
        let batch = try bridge.prepare(.snapshot(ExperienceViewModelSnapshot(values: [
    ...
    func testSnapshotResolvesListIdentitiesBeforeAmbiguousItemValues() throws {
        ...
        let bridge = try ExperienceRuntimeStateBridge(
            ...
            screenID: "screen-1",
            bootstrap: fixture.bootstrap,
            coordinator: ExperienceViewModelStateCoordinator(screens: fixture.screens)
        )
    ...
    func testSnapshotCreatesListItemLocalsBeforeApplyingFlattenedItemValues() throws {
        ...
        XCTAssertEqual(batch.newInstances, [
            ExperienceRuntimeNewInstance(localID: 1, schemaName: "Item", authoredInstanceName: nil),
            ExperienceRuntimeNewInstance(localID: 2, schemaName: "Item", authoredInstanceName: nil),
        ])
        XCTAssertEqual(batch.mutations, [
            .listClear(instance: .existing(instanceID(1)), path: "items"),
            .listInsert(
    ...
    func testNumberWritesUseTheExactRuntimeF32RepresentationForEchoSuppression() throws {
        ...
        XCTAssertTrue(try bridge.reconcile(ExperienceRuntimeOperationResult(
            renderOutcome: .notRequested,
            isDirty: true,
            isSettled: true,
            orderedOutputs: [
                output(sequence: 1, change: ExperienceRuntimeStateChange(
    ...
    func testImageWritesResolveAuthoredStringIdentitiesBeforeMutation() throws {
        let fixture = makeFixture()
        let imageProperty = ExperienceRuntimeSchemaProperty(
            schemaID: "Main",
        ...
        let schemas = fixture.bootstrap.catalog.schemas.map { schema in
            ...
            return ExperienceRuntimeSchema(
                id: schema.id,
                name: schema.name,
        ...
        let resolver = try ExperienceRuntimeImageIdentityResolver(images: [
            NuxPackageImageAsset(
                location: .external(key:
                    "assets/sha256/\(String(repeating: "a", count: 64)).png"
                ),
                riveAssetId: 7,
        ...
        let batch = try bridge.prepare(.value(
            ...
            value: "assets/sha256/\(String(repeating: "a", count: 64)).png",
            instanceID: "main-remote"
        ))
        ...
        let emitted = try bridge.reconcile(ExperienceRuntimeOperationResult(
            renderOutcome: .notRequested,
            isDirty: true,
            isSettled: true,
            orderedOutputs: [
        ...
        let imageKey = "assets/sha256/\(String(repeating: "a", count: 64)).png"
        XCTAssertEqual(emitted.first?.value as? String, imageKey)
        XCTAssertEqual(
    ...
    func testNestedValueAndTriggerInputsUseTheCatalogTypeAndStableRoot() throws {
        let fixture = makeFixture()
        let coordinator = ExperienceViewModelStateCoordinator(screens: fixture.screens)
        let nestedBridge = try ExperienceRuntimeStateBridge(
            screens: fixture.screens,
            screenID: "screen-1",
            bootstrap: fixture.bootstrap,
            coordinator: coordinator
        ...
        let triggerBridge = try ExperienceRuntimeStateBridge(
            screens: fixture.screens,
            screenID: "screen-1",
            bootstrap: fixture.bootstrap,
            coordinator: coordinator
    ...
    func testReconcileSuppressesOnlyExactEchoesAndAppliesOrderedRuntimeChanges() throws {
        ...
        let emitted = try bridge.reconcile(ExperienceRuntimeOperationResult(
            ...
            orderedOutputs: [
                output(
                    sequence: 10,
                    change: ExperienceRuntimeStateChange(
                        instanceID: instanceID(1),
                        path: "title",
    ...
    func testListInsertCreatesAndSettlesOneStableRemoteItemIdentity() throws {
        ...
        let emitted = try bridge.reconcile(ExperienceRuntimeOperationResult(
            ...
            isSettled: true,
            orderedOutputs: [
                output(sequence: 20, change: ExperienceRuntimeStateChange(
                    instanceID: settledID,
                    path: "label",
                    value: .string("C"),
                    originMutationID: batch.hostMutationID
                )),
                output(sequence: 21, change: ExperienceRuntimeStateChange(
                    instanceID: settledID,
                    path: "rank",
                    value: .number(2),
                    originMutationID: batch.hostMutationID
                )),
                output(sequence: 22, change: ExperienceRuntimeStateChange(
                    instanceID: instanceID(1),
                    path: "items",
                    value: nil,
                    originMutationID: batch.hostMutationID
            ...
            createdInstances: [
                ExperienceRuntimeCreatedInstance(localID: 1, instanceID: settledID),
            ]
        ))
    ...
    func testIdentityPreservingListReordersUseExactValidatedIndexes() throws {
        let fixture = makeFixture()
        func bridge() throws -> ExperienceRuntimeStateBridge {
            try ExperienceRuntimeStateBridge(
                screens: fixture.screens,
                screenID: "screen-1",
                bootstrap: fixture.bootstrap,
                coordinator: ExperienceViewModelStateCoordinator(screens: fixture.screens)
        ...
        XCTAssertThrowsError(try bridge().prepare(.list(
            ...
            XCTAssertEqual(
                error as? ExperienceRuntimeStateBridgeError,
    ...
    func testListMutationReindexesEveryAuthoredListIndexProperty() throws {
        ...
        let bootstrap = bootstrap(
            fixture.bootstrap,
            appending: ExperienceRuntimeSchemaProperty(
                schemaID: "Item",
                propertyID: "position",
                name: "position",
                kind: .listIndex
    ...
    func testOuterViewModelReplacementCreatesSettlesAndReusesStableIdentity() throws {
        ...
        _ = try bridge.reconcile(ExperienceRuntimeOperationResult(
            renderOutcome: .notRequested,
            isDirty: true,
            isSettled: true,
            createdInstances: [
                ExperienceRuntimeCreatedInstance(localID: 1, instanceID: instanceID(4)),
            ]
        ))
    ...
    func testOuterViewModelReplacementEchoDoesNotSuppressAnotherChildIdentity() throws {
        ...
        XCTAssertThrowsError(try bridge.reconcile(ExperienceRuntimeOperationResult(
            renderOutcome: .notRequested,
            isDirty: true,
            isSettled: true,
            orderedOutputs: [
    ...
    func testRuntimeOriginOuterViewModelReplacementUsesStableCanonicalIdentity() throws {
        ...
        let result = ExperienceRuntimeOperationResult(
            ...
            isSettled: true,
            orderedOutputs: [
                output(sequence: 2, change: ExperienceRuntimeStateChange(
                    instanceID: instanceID(1),
                    path: "child",
                    value: nil,
                    viewModelReference: ExperienceRuntimeViewModelReference(
    ...
    func testSnapshotRejectsConflictingFlattenedOuterViewModelEnvelope() throws {
        ...
        XCTAssertThrowsError(try bridge.prepare(.snapshot(ExperienceViewModelSnapshot(values: [
            value(path: "child/vmInstanceId", "child-a"),
    ...
    func testRuntimeOriginListChangeUsesAuthoritativeValuesAndReordersCanonicalRows() throws {
        ...
        _ = try bridge.prepare(.snapshot(ExperienceViewModelSnapshot(values: [
    ...
    func testRuntimeOriginListChangeFailsClosedWithoutAuthoritativeValues() throws {
        ...
        XCTAssertThrowsError(try bridge.reconcile(ExperienceRuntimeOperationResult(
            ...
            guard case .inconsistentResult(let message) = error as? ExperienceRuntimeStateBridgeError else {
    ...
    func testAbandonedPreparationDoesNotCommitPreferredExistingIdentity() throws {
        ...
        XCTAssertThrowsError(try bridge.prepare(.value(
            ...
            guard case .invalidInput(let message) = error as? ExperienceRuntimeStateBridgeError else {
                return XCTFail("Unexpected error: \(error)")
            }
    ...
    func testRuntimeTriggerOutputBecomesOneCanonicalTrueDelta() throws {
        ...
        let emitted = try bridge.reconcile(ExperienceRuntimeOperationResult(
            ...
            isSettled: true,
            orderedOutputs: [
                output(sequence: 30, change: ExperienceRuntimeStateChange(
                    instanceID: instanceID(1),
                    path: "pulse",
                    value: .enumeration(4),
                    originMutationID: nil
    ...
    func testPendingBatchMustBeReconciledOrExplicitlyAbandoned() throws {
        ...
        let input = ExperienceRuntimeCanonicalStateInput.value(
            path: VmPathRef(viewModelName: "Main", path: "title"),
            value: "Value",
            instanceID: "main-remote"
        )
        ...
        XCTAssertThrowsError(try bridge.prepare(input)) { error in
            XCTAssertEqual(error as? ExperienceRuntimeStateBridgeError, .operationPending)
        }
        bridge.abandonPendingBatch()
...
private extension ExperienceRuntimeStateBridgeTests {
    struct Fixture {
        let screens: JourneyDocument
        let bootstrap: ExperienceRuntimeBootstrap
    }

    func instanceID(_ rawValue: UInt64) -> ExperienceRuntimeInstanceID {
    ...
    func arenaByReversingFixtureItems(
        ...
        nodes[7] = ExperienceRuntimeValueNode(value: .list(items: [
            ExperienceRuntimeValueEdge(key: nil, nodeIndex: 10),
            ExperienceRuntimeValueEdge(key: nil, nodeIndex: 8),
        ]))
        return ExperienceRuntimeValueArena(nodes: nodes, roots: arena.roots)
    }

    func arenaBySettingChild(
        _ arena: ExperienceRuntimeValueArena,
        instanceID childInstanceID: ExperienceRuntimeInstanceID,
        name: String
    ) -> ExperienceRuntimeValueArena {
        var nodes = arena.nodes
        let childIndex = nodes.count
        let nameIndex = childIndex + 1
        nodes.append(ExperienceRuntimeValueNode(value: .viewModel(
            schemaID: "Child",
            instanceID: childInstanceID,
            fields: [ExperienceRuntimeValueEdge(key: "name", nodeIndex: nameIndex)]
        )))
        nodes.append(ExperienceRuntimeValueNode(value: .scalar(.string(name))))
        guard case .viewModel(let schemaID, let instanceID, let fields) = nodes[0].value else {
            return arena
        }
        nodes[0] = ExperienceRuntimeValueNode(value: .viewModel(
            schemaID: schemaID,
            instanceID: instanceID,
            fields: fields + [ExperienceRuntimeValueEdge(key: "child", nodeIndex: childIndex)]
        ))
        return ExperienceRuntimeValueArena(
            nodes: nodes,
            roots: arena.roots + [
                ExperienceRuntimeValueRoot(instanceID: childInstanceID, nodeIndex: childIndex),
            ]
        )
    }

    func bootstrapByRemovingFixtureItems(
        _ bootstrap: ExperienceRuntimeBootstrap
    ...
    func bootstrap(
        _ bootstrap: ExperienceRuntimeBootstrap,
        appending property: ExperienceRuntimeSchemaProperty,
    ...
    func bootstrapWithChildViewModel(
        ...
        let childProperty = ExperienceRuntimeSchemaProperty(
            schemaID: "Main",
            propertyID: "child",
            name: "child",
            kind: .viewModel,
        ...
        return ExperienceRuntimeBootstrap(
            ...
            catalog: ExperienceRuntimeCatalog(
                schemas: schemas,
                templates: bootstrap.catalog.templates + [
                    ExperienceRuntimeInstanceTemplate(
                        schemaID: "Child",
                        authoredName: nil,
                        authoredIndex: 0
                    ),
    ...
    func makeFixture() -> Fixture {
        let mainProperties: [(String, ExperienceRuntimeSchemaPropertyKind)] = [
            ("title", .string),
            ("count", .number),
            ("enabled", .bool),
            ("pulse", .trigger),
        ...
        let nodes = [
            ExperienceRuntimeValueNode(value: .viewModel(
                ...
                fields: [
                    ...
                    ExperienceRuntimeValueEdge(key: "count", nodeIndex: 2),
                    ExperienceRuntimeValueEdge(key: "enabled", nodeIndex: 3),
                    ExperienceRuntimeValueEdge(key: "pulse", nodeIndex: 4),
                    ExperienceRuntimeValueEdge(key: "settings", nodeIndex: 5),
                    ExperienceRuntimeValueEdge(key: "items", nodeIndex: 7),
                    ExperienceRuntimeValueEdge(key: "safeArea", nodeIndex: 12),
                    ExperienceRuntimeValueEdge(key: "nuxieTextInputs", nodeIndex: 14),
                ]
            )),
            ExperienceRuntimeValueNode(value: .scalar(.string("Before"))),
            ExperienceRuntimeValueNode(value: .scalar(.number(0))),
            ExperienceRuntimeValueNode(value: .scalar(.bool(false))),
            ExperienceRuntimeValueNode(value: .scalar(.enumeration(0))),
            ExperienceRuntimeValueNode(value: .object(
                schemaID: nil,
                fields: [ExperienceRuntimeValueEdge(key: "subtitle", nodeIndex: 6)]
            )),
            ExperienceRuntimeValueNode(value: .scalar(.string("Before subtitle"))),
            ExperienceRuntimeValueNode(value: .list(items: [
                ExperienceRuntimeValueEdge(key: nil, nodeIndex: 8),
                ExperienceRuntimeValueEdge(key: nil, nodeIndex: 10),
            ])),
            ExperienceRuntimeValueNode(value: .viewModel(
                ...
                fields: [ExperienceRuntimeValueEdge(key: "label", nodeIndex: 9)]
            )),
            ExperienceRuntimeValueNode(value: .scalar(.string("A"))),
            ExperienceRuntimeValueNode(value: .viewModel(
                schemaID: "Item",
                instanceID: instanceID(3),
                fields: [ExperienceRuntimeValueEdge(key: "label", nodeIndex: 11)]
            )),
            ExperienceRuntimeValueNode(value: .scalar(.string("B"))),
            ExperienceRuntimeValueNode(value: .object(
                schemaID: nil,
                fields: [ExperienceRuntimeValueEdge(key: "top", nodeIndex: 13)]
            )),
            ExperienceRuntimeValueNode(value: .scalar(.number(0))),
            ExperienceRuntimeValueNode(value: .viewModel(
                schemaID: nil,
                instanceID: nil,
                fields: [ExperienceRuntimeValueEdge(key: "value", nodeIndex: 15)]
            )),
            ExperienceRuntimeValueNode(value: .scalar(.string(""))),
        ]
        let arena = ExperienceRuntimeValueArena(
            nodes: nodes,
            roots: [
                ExperienceRuntimeValueRoot(instanceID: instanceID(1), nodeIndex: 0),
                ExperienceRuntimeValueRoot(instanceID: instanceID(2), nodeIndex: 8),
                ExperienceRuntimeValueRoot(instanceID: instanceID(3), nodeIndex: 10),
            ]
        )
        let catalog = ExperienceRuntimeCatalog(
            schemas: schemas,
            templates: [
                ExperienceRuntimeInstanceTemplate(schemaID: "Main", authoredName: "Default", authoredIndex: 0),
                ExperienceRuntimeInstanceTemplate(schemaID: "Item", authoredName: nil, authoredIndex: 0),
            ],
            instances: [
        ...
        let bootstrap = ExperienceRuntimeBootstrap(
            player: ExperienceRuntimePlayerMetadata(
                ...
                artboardName: "Entry",
                playerName: "Main",
                bounds: ExperienceRuntimeArtboardBounds(minX: 0, minY: 0, maxX: 100, maxY: 100)
            ),
            catalog: catalog,
            values: arena
        )
        ...
        return Fixture(screens: screens, bootstrap: bootstrap)
    }
}
