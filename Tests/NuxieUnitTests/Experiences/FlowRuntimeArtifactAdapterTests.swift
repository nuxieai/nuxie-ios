    ...
    override class func spec() {
        describe("FlowRuntimeArtifactAdapter") {
            it("materializes container-neutral assets in manifest order") {
                ...
                let requiredHash = ExperienceArtifactStore.sha256Hex(requiredBytes)
                ...
                let acquisitionFlow = Experience(
                    screens: RemoteFlow(
                        id: "acquired-flow",
            ...
            it("enforces the running asset budget before further hashing") {
                ...
                let manifest = try Self.manifest(
                    requiredHash: ExperienceArtifactStore.sha256Hex(firstBytes),
                    optionalHash: ExperienceArtifactStore.sha256Hex(secondBytes),
                    secondRequired: true
                )
                let temporaryDirectory = FileManager.default.temporaryDirectory
                    .appendingPathComponent(UUID().uuidString, isDirectory: true)
            ...
            it("omits an oversized optional asset without starving a later required asset") {
                ...
                let manifest = try Self.manifest(
                    requiredHash: ExperienceArtifactStore.sha256Hex(optionalBytes),
                    optionalHash: ExperienceArtifactStore.sha256Hex(requiredBytes),
                    firstRequired: false,
                    secondRequired: true
                )
                let temporaryDirectory = FileManager.default.temporaryDirectory
            ...
            it("omits an oversized optional asset without starving a later optional asset") {
                ...
                let manifest = try Self.manifest(
                    requiredHash: ExperienceArtifactStore.sha256Hex(oversizedBytes),
                    optionalHash: ExperienceArtifactStore.sha256Hex(validBytes),
                    firstRequired: false,
