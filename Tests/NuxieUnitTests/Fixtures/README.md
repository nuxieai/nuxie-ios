...
`native_input_owner.riv` is generated by `repeated_owned_native_input_fields()`
in runtime `crates/nux-capi/tests/support/semantic_text.rs` at `c60274ffb`.
Compile a small caller against that revision's `nuxie-schema` and write the
returned bytes directly. SHA-256:
`5c5bec1ef587cc7b08af3b1736762abc226878fa8063b97515cebe954cecc61a`.
The first artboard has two native fields and an authored default ViewModel with
an `answer` string. The SDK opens it with default binding enabled and checks
that a field-owner handle mutates that original model, not a copy. No font is
embedded; this qualifies state ownership, not rendered editing or IME.
