...
_Static_assert(NUX_CAPI_ABI_VERSION == 4, "unexpected portable ABI version");
...
void typecheck_nux_capi(const uint8_t *bytes, size_t len) {
    ...
    result = NULL;
    (void)nux_file_import_metal(renderer, bytes, len, NULL, &file, &result);
    (void)nux_capi_result_free(result);
    ...
    (void)nux_product_file_import_configured(
        renderer, bytes, len, &config, &file, &result);
    (void)nux_capi_result_free(result);
    (void)nux_file_free(file);
