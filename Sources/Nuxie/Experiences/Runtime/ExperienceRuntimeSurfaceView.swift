...
final class ExperienceRuntimeSurfaceView: UIView {
    ...
    private func deliver(_ touches: Set<UITouch>, as kind: ExperienceRuntimePointerKind) {
    ...
    func pointerEvent(
        for touch: UITouch,
        as kind: ExperienceRuntimePointerKind
...
private struct ExperienceRuntimePendingPointerInput {
    static let maximumEventCount = ScreenSessionLimits.pointerEvents * 2

    private var batches: [[ExperienceRuntimePointerEvent]] = []
    private var eventCount = 0
    private var activePointerIDs: Set<Int32> = []

    var isEmpty: Bool { batches.isEmpty }

    mutating func enqueue(_ events: [ExperienceRuntimePointerEvent]) throws {
        var nextBatch: [ExperienceRuntimePointerEvent] = []
    ...
    mutating func takeBatch() -> [ExperienceRuntimePointerEvent]? {
        guard !batches.isEmpty else { return nil }
        let batch: [ExperienceRuntimePointerEvent]
        if batches[0].count <= ScreenSessionLimits.pointerEvents {
            batch = batches.removeFirst()
        } else {
            batch = Array(batches[0].prefix(ScreenSessionLimits.pointerEvents))
            batches[0].removeFirst(ScreenSessionLimits.pointerEvents)
        }
    ...
    private func requireCapacity(forAdditionalEvents count: Int) throws {
        guard hasCapacity(forAdditionalEvents: count) else {
            throw ExperienceRuntimeDisplayHostError.pendingPointerInputOverflow(
    ...
    private mutating func removeSupersededMove(
        pointerID: Int32,
        from nextBatch: inout [ExperienceRuntimePointerEvent]
    ) -> Bool {
    ...
    private func pendingMoveIndex(
        pointerID: Int32,
        in events: [ExperienceRuntimePointerEvent]
    ) -> Int? {
...
typealias ExperienceRuntimeOperationCompletion = @MainActor (
    Result<ExperienceRuntimeOperationResult, Error>
) -> Void

typealias ExperienceRuntimeTextRunCompletion = ExperienceRuntimeOperationCompletion

enum ExperienceRuntimeDisplayResultSource: Equatable, Sendable {
    case stateBatch
    case textRun
    case textRender
    case pointerBatch
...
private struct ExperienceRuntimePendingStateBatches {
    struct Entry {
        let prepare: @MainActor () throws -> ExperienceRuntimeStateBatch
        let completion: ExperienceRuntimeOperationCompletion
    }
...
private struct ExperienceRuntimePendingTextRuns {
    struct Entry {
        var mutation: ExperienceRuntimeTextRunMutation
        var completions: [ExperienceRuntimeTextRunCompletion]
    }
    ...
    mutating func enqueue(
        _ mutation: ExperienceRuntimeTextRunMutation,
        completion: @escaping ExperienceRuntimeTextRunCompletion
    ) {
...
@MainActor
final class ExperienceRuntimeDisplayHost: NSObject {
    ...
    private enum PendingOperation {
        case detach
        case reattach(ExperienceRuntimeAppleSurfaceTarget)
        case recoveryDetach
        case recoveryReattach(ExperienceRuntimeAppleSurfaceTarget)
        case recoveryRedraw(ExperienceRuntimeFrameTime)
        case resize(ExperienceRuntimeSurfaceSize)
        case stateBatch(ExperienceRuntimePendingStateBatches.Entry)
        case textRun(ExperienceRuntimePendingTextRuns.Entry)
        case textRender(ExperienceRuntimeFrameTime)
        case pointerBatch([ExperienceRuntimePointerEvent])
        case offscreenAdvance(ExperienceRuntimeFrameTime)
        case frame(ExperienceRuntimeFrameTime)
    }
    ...
    private let onResult: @MainActor (
        _ original: ExperienceRuntimeOperationResult,
        _ projected: ExperienceRuntimeOperationResult,
        _ source: ExperienceRuntimeDisplayResultSource
    ) -> Void
    private let onError: @MainActor (Error) -> Void
    private let drawableGate: ExperienceRuntimeDrawableGate
    private let usesSystemDisplayLink: Bool

    private var surface: ScreenRenderSurface?
    // nonisolated(unsafe): MainActor-confined; also read by deinit, which has
    // exclusive access to the last reference.
    private nonisolated(unsafe) var displayLink: CADisplayLink?
    private var displayLinkProxy: ExperienceRuntimeDisplayLinkProxy?
    ...
    private var pendingPointerInput = ExperienceRuntimePendingPointerInput()
    private var pendingStateBatches = ExperienceRuntimePendingStateBatches()
    private var pendingTextRuns = ExperienceRuntimePendingTextRuns()
    ...
    init(
        ...
        resultProjector: @escaping @MainActor (
            ExperienceRuntimeOperationResult
        ) -> ExperienceRuntimeOperationResult = { $0 },
    ...
    convenience init(
        ...
        onResult: @escaping @MainActor (ExperienceRuntimeOperationResult) -> Void,
        onError: @escaping @MainActor (Error) -> Void = { _ in }
    ) {
        self.init(
            session: session,
    ...
    func start() async throws {
        ...
        guard let surfaceView else {
            let error = ExperienceRuntimeHostError.disposedSurface
    ...
    func setText(
        ...
        completion: @escaping ExperienceRuntimeTextRunCompletion = { _ in }
    ) {
        ...
        pendingTextRuns.enqueue(
            ExperienceRuntimeTextRunMutation(name: name, text: text),
    ...
    func performStateBatch(
        prepare: @escaping @MainActor () throws -> ExperienceRuntimeStateBatch,
        completion: @escaping ExperienceRuntimeOperationCompletion = { _ in }
    ) {
        if let terminalError {
            completion(.failure(terminalError))
            return
        ...
        pendingStateBatches.enqueue(
            ExperienceRuntimePendingStateBatches.Entry(
                prepare: prepare,
                completion: completion
            )
        )
    ...
    private func updateDisplayLinkForCurrentScreen() {
        ...
        invalidateDisplayLink()
        let proxy = ExperienceRuntimeDisplayLinkProxy(host: self)
    ...
    private func applicationDidReceiveMemoryWarning() {
        ...
        case .disposed:
            reportTerminalFailure(ExperienceRuntimeHostError.disposedSurface)
            return
        }
        reconcile()
    }
    ...
    private func nextOperation(for surface: ScreenRenderSurface?) -> PendingOperation? {
    ...
    private func acceptRuntimeSchedule(
        from result: ExperienceRuntimeOperationResult,
    ...
    private func perform(
        ...
        on surface: ScreenRenderSurface?
    ) async throws {
        switch operation {
        case .detach:
            guard let surface else { throw ExperienceRuntimeHostError.disposedSurface }
            _ = try await surface.detach()
            lastAppliedSize = nil
            frameClock.reset()
        ...
        case .stateBatch(let entry):
            let batch: ExperienceRuntimeStateBatch
            do {
        ...
        case .textRun(let entry):
            do {
                let result = try await session.perform(
                    .textRunBatch(ExperienceRuntimeTextRunBatch(mutations: [entry.mutation]))
                )
                acceptRuntimeSchedule(
                    from: result,
                    requestsVisibleRender: result.isDirty
    ...
    private func acquireDrawable(
        for surface: ScreenRenderSurface
    ...
    private func surfaceTarget(
        for view: ExperienceRuntimeSurfaceView
    ) -> ExperienceRuntimeAppleSurfaceTarget {
        let scale = view.window?.screen.scale ?? view.contentScaleFactor
        view.metalLayer.contentsScale = scale
        return ExperienceRuntimeAppleSurfaceTarget(
            layer: view.metalLayer,
            size: ExperienceRuntimeSurfaceSizing.pixels(
    ...
    private func pointerTransform(
        for view: ExperienceRuntimeSurfaceView
    ) -> ExperienceContainCenterTransform? {
        let bounds = session.bootstrap.player.bounds
        return ExperienceContainCenterTransform(
    ...
    private func handleRecoverableOperationFailure(_ error: Error) -> Bool {
        guard let hostError = error as? ExperienceRuntimeHostError,
              case .recoverableSurface(.deviceLost) = hostError else {
            return false
        }
        guard !deviceLossAwaitingSuccessfulFrame else {
            reportTerminalFailure(
                ExperienceRuntimeHostError.unrecoverableSurface(.deviceLost)
            )
            return true
        }
    ...
    private func reservePendingHostWork(
        for completion: ExperienceRuntimeOperationCompletion
    ) -> Bool {
        guard pendingHostWorkCount < ScreenSessionLimits.batchItems else {
            completion(.failure(ExperienceRuntimeDisplayHostError.pendingHostWorkOverflow(
...
extension ExperienceRuntimeDisplayHost: ExperienceRuntimeSurfaceViewObserver {}

@MainActor
private final class ExperienceRuntimeDisplayLinkProxy: NSObject {
    weak var host: ExperienceRuntimeDisplayHost?

    init(host: ExperienceRuntimeDisplayHost) {
        self.host = host
    }
