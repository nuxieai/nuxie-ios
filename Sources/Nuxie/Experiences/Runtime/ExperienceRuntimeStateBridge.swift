...
enum ExperienceRuntimeCanonicalStateInput {
    case snapshot(ExperienceViewModelSnapshot)
...
enum ExperienceRuntimeStateBridgeError: LocalizedError, Equatable {
...
final class ExperienceRuntimeStateBridge {
    ...
    private struct PendingBatch {
        ...
        let preparedRemoteToRuntime: [RemoteInstance: ExperienceRuntimeInstanceID]
    }

    private struct PendingEcho: Equatable {
        let instance: ExperienceRuntimeInstanceReference?
        let path: String
        let value: ExperienceRuntimeScalarValue?
        let viewModelReference: ExperienceRuntimeInstanceReference?
    }
    ...
    private struct ListKey: Hashable {
        let owner: ExperienceRuntimeInstanceID
        let path: String
    }

    private struct PendingListCommit: Equatable {
        let key: ListKey
        let items: [ExperienceRuntimeInstanceReference]
    }
    ...
    private struct ResolvedProperty {
        ...
        init(_ property: ExperienceRuntimeSchemaProperty) {
            kind = property.kind
            enumValues = property.enumValues
            referencedSchemaID = property.referencedSchemaID
        }

        init(inferredKind: ExperienceRuntimeSchemaPropertyKind) {
            kind = inferredKind
            enumValues = []
    ...
    private let screen: JourneyScreen
    private let coordinator: ExperienceViewModelStateCoordinator
    private let bootstrap: ExperienceRuntimeBootstrap
    private let imageIdentityResolver: ExperienceRuntimeImageIdentityResolver?
    private let schemasByID: [String: ExperienceRuntimeSchema]
    private let schemasByName: [String: [ExperienceRuntimeSchema]]
    private var instancesByID: [ExperienceRuntimeInstanceID: ExperienceRuntimeInstance]
    private var remoteToRuntime: [RemoteInstance: ExperienceRuntimeInstanceID] = [:]
    private var nextMutationID: UInt64 = 1
    private var nextLocalID: UInt32 = 1
    private var listItemsByKey: [ListKey: [ExperienceRuntimeInstanceID]] = [:]
    private var activeSnapshotSchemaIDsByRemoteID: [String: String] = [:]
    private var pending: PendingBatch?

    init(
        screens: JourneyDocument,
        screenID: String,
        bootstrap: ExperienceRuntimeBootstrap,
        coordinator: ExperienceViewModelStateCoordinator,
        imageIdentityResolver: ExperienceRuntimeImageIdentityResolver? = nil
    ) throws {
        guard let screen = screens.screens.first(where: { $0.id == screenID }) else {
            throw ExperienceRuntimeStateBridgeError.invalidInput(
                "Remote experience has no screen named '\(screenID)'"
            )
        }
    ...
    func prepare(_ input: ExperienceRuntimeCanonicalStateInput) throws -> ExperienceRuntimeStateBatch {
        guard pending == nil else {
            throw ExperienceRuntimeStateBridgeError.operationPending
        }
        ...
        var newInstances: [ExperienceRuntimeNewInstance] = []
        var pendingNewInstances: [PendingNewInstance] = []
        var listCommits: [PendingListCommit] = []
        switch input {
        case .snapshot(let snapshot):
            var snapshotMutations: [ExperienceRuntimeStateMutation] = []
        ...
        return ExperienceRuntimeStateBatch(
    ...
    private func prepareCanonicalValue(
        _ rawValue: Any,
        path: String,
        schema: ExperienceRuntimeSchema,
        reference: ExperienceRuntimeInstanceReference,
        newInstances: inout [ExperienceRuntimeNewInstance],
        pendingNewInstances: inout [PendingNewInstance],
        listCommits: inout [PendingListCommit]
    ) throws -> [ExperienceRuntimeStateMutation] {
        ...
        case .object:
            ...
            var mutations: [ExperienceRuntimeStateMutation] = []
        ...
        case .list:
            ...
            var finalItems: [ExperienceRuntimeInstanceReference] = []
            var mutations: [ExperienceRuntimeStateMutation] = [
                .listClear(instance: reference, path: path),
            ]
            for (index, value) in values.enumerated() {
                let preferred: ExperienceRuntimeInstanceID? = if current.indices.contains(index) {
    ...
    private func currentNestedViewModelInstance(
        owner: ExperienceRuntimeInstanceID,
        path: String
    ) -> ExperienceRuntimeInstanceID? {
    ...
    private func normalizeOuterViewModelEnvelopes(
        ...
        var normalized: [JourneyViewModelValue] = []
        ...
        for (index, value) in values.enumerated() {
            ...
            normalized.append(JourneyViewModelValue(
    ...
    private func resolveSnapshotReference(
        ...
        newInstances: inout [ExperienceRuntimeNewInstance],
        pendingNewInstances: inout [PendingNewInstance]
    ) throws -> ExperienceRuntimeInstanceReference {
        ...
        newInstances.append(ExperienceRuntimeNewInstance(
            localID: localID,
    ...
    private func prepareListOperation(
        ...
        let owner = ExperienceRuntimeInstanceReference.existing(target.instanceID)
    ...
    private func listIndexMutations(
        for items: [ExperienceRuntimeInstanceReference],
    ...
    private func prepareReferencedInstance(
        _ rawValue: Any?,
        preferredExisting: ExperienceRuntimeInstanceID? = nil,
        newInstances: inout [ExperienceRuntimeNewInstance],
        pendingNewInstances: inout [PendingNewInstance]
    ) throws -> (
        reference: ExperienceRuntimeInstanceReference,
        ...
        let remote = RemoteInstance(id: remoteID, schemaID: schema.id)
        let reference: ExperienceRuntimeInstanceReference
        if let existing = remoteToRuntime[remote] {
    ...
    private func schemaIDHint(for remoteID: String) throws -> String? {
        ...
        for value in screens.viewModelValues ?? [] where value.instanceId == remoteID {
    ...
    private func currentListItems(
        ...
        if let cached = listItemsByKey[key] {
            return cached.map(ExperienceRuntimeInstanceReference.existing)
        }
        guard let root = bootstrap.values.roots.first(where: {
        ...
        let identities = try edges.map { edge -> ExperienceRuntimeInstanceID in
        ...
        return identities.map(ExperienceRuntimeInstanceReference.existing)
    }
    ...
    private func requiredInteger(_ rawValue: Any?, label: String) throws -> Int {
        guard let value = try optionalInteger(rawValue, label: label) else {
            throw ExperienceRuntimeStateBridgeError.invalidInput("\(label) is required")
        }
        return value
    }
    ...
    private func optionalInteger(_ rawValue: Any?, label: String) throws -> Int? {
        ...
        let value = unwrap(rawValue)
        if value is Bool {
            throw ExperienceRuntimeStateBridgeError.invalidInput("\(label) must be an integer")
        }
    ...
    func reconcile(
    ...
    ) throws -> [ExperienceRendererViewModelChange] {
        ...
        var emitted: [ExperienceRendererViewModelChange] = []
        ...
        for change in prepared {
            guard coordinator.setValue(
                ...
                throw ExperienceRuntimeStateBridgeError.inconsistentResult(
                    "Canonical coordinator rejected runtime path '\(change.path.path)'"
                )
            }
            emitted.append(ExperienceRendererViewModelChange(
                path: change.path,
                value: change.value,
                source: "runtime",
                screenId: change.screenID,
    ...
    private func validateSettlements(
        _ created: [ExperienceRuntimeCreatedInstance],
        pending: PendingBatch?,
        instancesByID: [ExperienceRuntimeInstanceID: ExperienceRuntimeInstance]
    ) throws -> [UInt32: ExperienceRuntimeInstanceID] {
    ...
    private func stageSettlements(
        _ settlements: [UInt32: ExperienceRuntimeInstanceID],
        pending: PendingBatch?,
        instancesByID: inout [ExperienceRuntimeInstanceID: ExperienceRuntimeInstance],
        remoteToRuntime: inout [RemoteInstance: ExperienceRuntimeInstanceID],
        listItemsByKey: inout [ListKey: [ExperienceRuntimeInstanceID]]
    ) throws {
        ...
        for requested in pending.newInstances {
            ...
            instancesByID[runtimeID] = ExperienceRuntimeInstance(
                id: runtimeID,
                schemaID: schema.id,
    ...
    private func expectedEcho(
        for mutation: ExperienceRuntimeStateMutation
    ) -> PendingEcho? {
        switch mutation {
    ...
    private func exactEcho(
        ...
        settlements: [UInt32: ExperienceRuntimeInstanceID],
        instancesByID: [ExperienceRuntimeInstanceID: ExperienceRuntimeInstance],
        matches change: ExperienceRuntimeStateChange
    ) -> Bool {
    ...
    private func prepareCanonicalChange(
        _ change: ExperienceRuntimeStateChange,
        authoritativeValues: ExperienceRuntimeValueArena?,
        instancesByID: [ExperienceRuntimeInstanceID: ExperienceRuntimeInstance],
        remoteToRuntime: [RemoteInstance: ExperienceRuntimeInstanceID],
        listItemsByKey: inout [ListKey: [ExperienceRuntimeInstanceID]]
    ) throws -> PreparedCanonicalChange {
        let runtimeInstance: ExperienceRuntimeInstance
    ...
    private func canonicalListRows(
        identities: [ExperienceRuntimeInstanceID],
        path: VmPathRef,
        remoteInstanceID: String?,
        remoteToRuntime: [RemoteInstance: ExperienceRuntimeInstanceID]
    ) throws -> [[String: Any]] {
    ...
    private func canonicalValue(
        _ value: ExperienceRuntimeScalarValue,
    ...
    private func resolveSchema(_ identity: String) throws -> ExperienceRuntimeSchema {
    ...
    private static func resolveSchema(
        _ identity: String,
        schemasByID: [String: ExperienceRuntimeSchema],
        schemasByName: [String: [ExperienceRuntimeSchema]]
    ) throws -> ExperienceRuntimeSchema {
        var matches: [ExperienceRuntimeSchema] = []
    ...
    private func resolveTarget(
        ...
        if let explicit = path.viewModelName {
            schemaIdentity = explicit
                  ...
                  let hinted = screens.viewModelValues?.first(where: {
    ...
    private func resolveCatalogProperty(
        segments: [String],
        rootSchema: ExperienceRuntimeSchema
    ...
    private func propertyKind(
        for value: ExperienceRuntimeValue
    ) -> ExperienceRuntimeSchemaPropertyKind {
        switch value {
