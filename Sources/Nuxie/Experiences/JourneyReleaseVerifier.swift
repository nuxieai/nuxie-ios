...
struct JourneyReleaseVerifier: Sendable {
    func validateIdentity(_ identity: JourneyReleaseIdentity) throws {
    ...
    func validateJourneyEnvelope(_ bytes: Data) throws {
        let envelope = try decodeEnvelope(bytes, mediaType: JourneyReleaseDescriptor.mediaType)
        ...
        guard let signature = canonicalBase64(envelope.signature.signatureBase64, maximumDecodedBytes: 64),
              signature.count == 64 else { throw JourneyReleaseAuthenticationError.invalidEnvelope }
    }

    private func authenticateEnvelope(
        _ envelopeBytes: Data, authorizationKeys: [JourneyPackageAuthorizationKey],
        mediaType: String, signatureDomain: String
    ) throws -> (JourneyReleaseEnvelope, Data) {
        ...
        guard let keyBytes = keys[envelope.signature.keyId] else {
            throw JourneyReleaseAuthenticationError.unknownKey(
        ...
        do {
            ...
            throw JourneyReleaseAuthenticationError.invalidAuthorizationKeys
        }
        guard let signatureBytes = canonicalBase64(
            ...
            throw JourneyReleaseAuthenticationError.invalidEnvelope
        }
    ...
    func authenticateJourney(
        ...
        let (envelope, bytes) = try authenticateEnvelope(
            ...
            signatureDomain: JourneyReleaseDescriptor.signatureDomain
        )
        ...
        do {
            ...
            descriptor = try ExactJSONCodec.decode(JourneyReleaseDescriptor.self, from: bytes)
            try validateIdentity(descriptor.identity)
        } catch let error as JourneyReleaseAuthenticationError { throw error }
        catch { throw JourneyReleaseAuthenticationError.invalidDescriptor }
        guard descriptor.identity == expectedIdentity, descriptor.leg.id == expectedLegId else {
            throw JourneyReleaseAuthenticationError.identityMismatch
        }
    ...
    private func validateReplayPolicy(
        _ policy: JourneyReleaseReplayPolicy,
    ...
    {
        guard bytes.count <= JourneyReleaseLimits.envelopeBytes else {
            throw JourneyReleaseAuthenticationError.descriptorLimitExceeded
        }
        do {
            ...
            guard envelope.mediaType == mediaType,
                  ...
                  envelope.signature.keyId.utf8.count
                    <= JourneyReleaseLimits.keyIDBytes,
                  isLowercaseSHA256(envelope.descriptorSha256),
                  envelope.descriptorSizeBytes >= 0,
                  envelope.descriptorSizeBytes
                    <= JourneyReleaseLimits.descriptorBytes else {
                throw JourneyReleaseAuthenticationError.invalidEnvelope
            }
            return envelope
        } catch let error as JourneyReleaseAuthenticationError {
            throw error
        } catch {
            throw JourneyReleaseAuthenticationError.invalidEnvelope
        }
    ...
    private func decodeDescriptorBytes(
        _ envelope: JourneyReleaseEnvelope
    ) throws -> Data {
        guard let bytes = canonicalBase64(
            envelope.descriptorBytesBase64,
            maximumDecodedBytes: JourneyReleaseLimits.descriptorBytes
        ) else {
            throw JourneyReleaseAuthenticationError.invalidEnvelope
        }
        ...
        guard SHA256Provider.hexDigest(bytes) == envelope.descriptorSha256 else {
            throw JourneyReleaseAuthenticationError.digestMismatch
        }
        return bytes
    }
    ...
    private func validateJSONValue(_ value: Any, field: String?) throws {
        ...
        if let values = value as? [Any] {
            let maximum: Int?
            switch field {
            case "screens": maximum = JourneyReleaseLimits.screenCount
            case "products": maximum = JourneyReleaseLimits.productCount
            case "placements": maximum = JourneyReleaseLimits.placementCount
            ...
            for (key, item) in object {
                if key == "sizeBytes", let number = item as? NSNumber {
                    guard CFGetTypeID(number) != CFBooleanGetTypeID(),
                          ...
                          number.doubleValue
                            <= Double(JourneyReleaseLimits.artifactAggregateBytes)
                    else {
                        throw JourneyReleaseAuthenticationError.malformedBounds(
                            "sizeBytes"
                        )
    ...
    private func validateRenderValue(_ value: Any?, field: String) throws {
        if let object = value as? [String: Any] {
                ...
                if ["path", "key", "sourceKey", "member"].contains(key),
                   let path = item as? String {
                    guard isSafeRelativeArtifactPath(path) else {
                        throw JourneyReleaseAuthenticationError.unsafeArtifactKey(
                            path
                        )
                    }
            ...
            throw JourneyReleaseAuthenticationError.unsafeArtifactKey(string)
        }
    }

    private func requiredCapabilities(
        in requirements: [String: JourneyReleaseJSONValue]
    ) throws -> [String] {
        guard let value = requirements["requiredCapabilities"] else { return [] }
        guard case .array(let values) = value,
              values.count <= JourneyReleaseLimits.requiredCapabilityCount else {
            throw JourneyReleaseAuthenticationError.malformedBounds(
        ...
        let capabilities = try values.map { value in
            guard case .string(let capability) = value,
                  !capability.isEmpty,
                  capability.utf8.count <= JourneyReleaseLimits.keyIDBytes else {
                throw JourneyReleaseAuthenticationError.malformedBounds(
                    "requiredCapabilities"
                )
            }
    ...
    private func validateRuntimeRequirements(
        _ requirements: [String: JourneyReleaseJSONValue],
        ...
        guard unsupported.isEmpty else {
            throw JourneyReleaseAuthenticationError.unsupportedCapabilities(
                unsupported
            )
        }
    }
    ...
    private func validateArtifactReferences(_ root: [String: Any]) throws {
        ...
        let aggregate = try uniqueSizesByDigest.values.reduce(0) { total, size in
            ...
            guard !overflow,
                  sum <= JourneyReleaseLimits.artifactAggregateBytes else {
                throw JourneyReleaseAuthenticationError.malformedBounds(
        ...
        guard aggregate <= JourneyReleaseLimits.artifactAggregateBytes else {
            throw JourneyReleaseAuthenticationError.malformedBounds(
                "artifactAggregateBytes"
            )
        }
            ...
            if object["sizeBytes"] != nil {
                ...
                if key == "renders/sha256/\(digest).riv" || key == "renders/sha256/\(digest).nux" {
                    ...
                    maximum = 4 * 1_024 * 1_024
                          ...
                          ["png", "jpg", "webp", "ttf", "otf", "bin", "mp4"]
                            .contains(String(fileExtension)) {
                    maximum = JourneyReleaseLimits.externalAssetBytes
                } else {
    ...
    private func validateKeys(
        _ keys: [JourneyPackageAuthorizationKey]
    ) throws -> [String: Data] {
        ...
        for key in keys {
            guard !key.keyID.isEmpty,
                  key.keyID.utf8.count <= JourneyReleaseLimits.keyIDBytes,
    ...
    private func isSafeRelativeArtifactPath(_ value: String) -> Bool {
        guard !value.isEmpty, value.utf8.count <= JourneyReleaseLimits.genericStringBytes,
