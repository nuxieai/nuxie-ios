...
enum FlowArtifactStoreError: LocalizedError, Sendable {
    ...
    var errorDescription: String? {
        ...
        case .missingManifest:
            return "Experience artifact manifest is missing"
        case .missingRivFile(let path):
            return "Experience artifact RIV file is missing: \(path)"
        ...
        case let .fileSizeMismatch(path, expected, actual):
            return "Experience artifact file size mismatch for \(path): expected \(expected), got \(actual)"
        case let .sha256Mismatch(path, expected, actual):
            return "Experience artifact SHA-256 mismatch for \(path): expected \(expected), got \(actual)"
        }
    }
}
...
actor ExperienceArtifactStore {
    static let manifestPath = "nuxie-manifest.json"
    static let manifestSignaturePath = ExperienceManifestSignature.artifactPath
    ...
    init(
        ...
        LogDebug("ExperienceArtifactStore initialized at: \(self.cacheDirectory.path)")
    }

    func preloadArtifact(for flow: Experience) async {
        do {
    ...
    func getCachedArtifact(for flow: Experience) async throws -> LoadedFlowArtifact? {
    ...
    private func getCachedArtifactWithoutCoordination(
        ...
        try validateBuildManifest(flow.screens.flowArtifact.manifest)
        let declaredManifestFile = flow.screens.flowArtifact.manifest.files.first(where: {
        ...
        try validateCachedBuildSidecars(
            flow.screens.flowArtifact.manifest,
            artifactManifest: manifest,
            directoryURL: directoryURL
        )
        try validateRivDeclarations(
            manifest: manifest,
            buildManifest: flow.screens.flowArtifact.manifest
        )
    ...
    func getOrDownloadArtifact(for flow: Experience) async throws -> LoadedFlowArtifact {
    ...
    private func authorizationEvidence(
        ...
        if FileManager.default.fileExists(atPath: signatureURL.path) {
            do {
                ...
                LogWarning("Experience manifest signature file could not be read: \(error)")
                signatureData = nil
            }
        } else {
            signatureData = nil
    ...
    private func boundedSignatureEvidence(at url: URL) throws -> Data {
        ...
        guard fileSize <= UInt64(limit) else {
            LogWarning("Experience manifest signature exceeds the runtime evidence limit")
            return Data()
        }

        let data = try handle.read(upToCount: limit + 1) ?? Data()
        guard data.count <= limit else {
            LogWarning("Experience manifest signature grew beyond the runtime evidence limit")
            return Data()
        }
        return data
    }
    ...
    private func downloadSignatureBuildFile(
        ...
        do {
            ...
            LogWarning("Experience manifest signature could not be acquired: \(error)")
        }
    }
    ...
    private func canonicalDirectoryURL(for flow: Experience) -> URL {
        let key = artifactCacheKey(for: flow)
        return cacheDirectory.appendingPathComponent(key)
    }

    private func artifactCacheKey(for flow: Experience) -> String {
        let artifact = flow.screens.flowArtifact
