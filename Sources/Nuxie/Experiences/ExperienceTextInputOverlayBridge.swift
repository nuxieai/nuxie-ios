...
private struct FlowTextInputGeometryProjection {
               ...
               change.path == "nuxieTextInputs" {
                ...
                do {
                    ...
                    issues.append(Issue(
                        ...
                        message: "ExperienceTextInputOverlayBridge: failed to rebind nuxieTextInputs: \(error)"
                    ))
                }
                continue
            }
            ...
            if let containerInstanceID,
               ...
               let inputID = entriesByInputID.values.first(where: {
                ...
                issues.append(Issue(
                    ...
                    message: "ExperienceTextInputOverlayBridge: input '\(inputID)' replaced its inner geometry ViewModel; inner replacement is unsupported"
                ))
                continue
            }
    ...
    private static func bindIssue(_ message: String) -> Issue {
        Issue(
            ...
            message: "ExperienceTextInputOverlayBridge: failed to bind runtime geometry: \(message)"
        )
    }
...
@MainActor
final class ExperienceTextInputOverlayBridge: NSObject, UITextFieldDelegate, UITextViewDelegate {
    ...
    func bind(
        ...
        for inputID in duplicateInputIDs.sorted(by: { lhs, rhs in
            ...
            emitDiagnostic(
                ...
                message: "ExperienceTextInputOverlayBridge: duplicate text input ID '\(inputID)' is disabled"
            )
        }
        ...
        do {
            ...
            emitDiagnostic(
                code: "nuxie_ios.text_input_geometry_bind_failed",
                message: "ExperienceTextInputOverlayBridge: failed to bind runtime geometry: \(error)"
            )
        }
        ...
        textWriter(renderedText, input.riveTextRunName) { [weak self] result in
            ...
            case .failure(let error):
                ...
                self.emitDiagnostic(
                    ...
                    message: "ExperienceTextInputOverlayBridge: failed to update text run '\(input.riveTextRunName)': \(error)"
                )
            }
            self.layout()
        }
