...
@_spi(Testing) public enum JourneyPresentationStateHost {
    ...
    public static func present(
        ...
        let host = try JourneyFixtureHost.makePresentationInputs(
            fixtureBaseURL: scenario.fixtureBaseURL,
            cacheRootURL: scenario.cacheRootURL
        )
        let fixture = try await host.authenticate()
        let screenID = try host.resolveInitialScreenID(fixture: fixture)
            ...
            artifactLoader: { _, _, requestedScreenID in
                ...
                case .warm:
                    guard let warmArtifact else {
                        throw JourneyPresentationStateHostError.acquisitionUnavailable
                    }
                    return warmArtifact
                case .normal:
...
@_spi(Testing) public enum JourneyPresentationStateHostError: LocalizedError {
