...
| Images and other file assets | Adapts embedded/out-of-band assets, supplies native image/font/audio decoders, and includes a generic URLSession CDN loader. | Swift verifies image/font files and resolves them by stable Rive `uniqueName`; dynamic image ViewModel properties resolve source key, unique name or path. Signed scene bytes may also contain embedded audio. | Images/fonts and stable identity required. Keep Swift network/cache/hash policy and Rust decoding/GPU upload. Accept embedded audio transitively authenticated by the signed scene and preserve it across renderer lifecycles; omit Rive CDN loading, external-audio delivery and product audio playback. |
...
| Audio | Creates an audio engine/listener and reacts to app activation for audio. | The manifest has no audio delivery schema. A pinned upstream fixture proves that embedded audio inside signed scene bytes survives presentation teardown/recovery. | Do not add a product audio engine, playback controls or activation policy in v1. Configured import accepts signed embedded audio as a native-owned file asset; external audio remains unsupported. |
...
- product/platform audio playback, controls, activation policy or external-audio delivery;
