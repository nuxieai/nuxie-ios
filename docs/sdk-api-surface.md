...
| `delegate: NuxieDelegate?` | Main-actor feature-access, curated activity-forwarding, and Run App Action callbacks. |
...
Hosts can forward a deliberately curated subset of SDK activity through
`NuxieDelegate.nuxieDidEmit(_:)`. `NuxieActivityInfo` exposes both an
exhaustively switchable `activity` and analytics-ready `name`/`properties`.
See [Forward Events Into Your Analytics](forward-events-into-your-analytics.md).
Designer-authored Run App Action steps arrive separately through
`nuxie(_:didRequestAppAction:)`; see [Run App Action](run-app-action.md).
